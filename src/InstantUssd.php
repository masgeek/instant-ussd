...
class InstantUssd
{
    ...
    public function __construct(array $instantUssdConfig, $initializer)
    {
        ...
        $ussdEventManager = $instantUssdConfig['ussd_event_manager'];
        $this->setUssdEventManager($ussdEventManager);
    ...
    public function getEventManager()
    {
        if (!$this->eventManager) {
            if (empty($this->ussdEventManager)) {
                throw new Exception('UssdEventManager class not set.');
            }
            $class = $this->ussdEventManager;
    ...
    /**
     *
     * @param string $ussdEventManager
     */
    protected function setUssdEventManager($ussdEventManager)
    {
        if (!(gettype($ussdEventManager) === 'string')) {
            throw new \Exception('ussd_event_manager should be a string.');
        }
        $this->ussdEventManager = $ussdEventManager;
    }
